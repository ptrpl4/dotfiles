...
- `n` itself is a Homebrew formula, but Node is not — it lives under `$N_PREFIX` (`~/.local`,
  set in `shell/path.sh`). Never `brew install node`, and don't expect Node in
  `brew list`/`brew leaves`
